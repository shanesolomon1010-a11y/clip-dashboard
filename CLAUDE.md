...
- **Vercel cron `/api/cron/youtube-sync` is the active production source for Shorts.** The LaunchAgent at `scripts/youtube-studio-sync.ts` is preserved-but-inactive (unloaded 2026-05-05). Don't `launchctl load` it back without checking with Shane — it would double-write and re-introduce the cumulative-vs-delta volatility we hit in April. Diagnostics endpoint's `last_scraper_run` will read RED forever; that's expected (lessons.md 2026-04-28 superseded 2026-05-17).
