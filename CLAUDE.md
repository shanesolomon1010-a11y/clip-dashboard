...
- **Auto-mapper is dead — BOTH upload-side signals are gone, mapping is manual.** `src/lib/shorts-discovery.ts:72-88` matches `MBM###-CLIP-###` against `snippet.tags`, which is always empty on this channel; and `fileDetails.fileName` is no longer returned by the YT Data API (verified 2026-05-29 against a known upload with `youtube.force-ssl` granted — the field is gone server-side, NOT scope-gated). **Do NOT revive a `fileDetails.fileName` auto-mapper or recommend re-auth to "fix" it.** Mapping is done manually via `map_clip()` + the Settings → Mapping UI + the `/tmp/clipmatch` image-match pipeline (banner-frame pHash vs reel thumbnails). New YT shorts still auto-create a PENDING row by design (expect daily growth of ~1 YT short + a few IG reels) until/unless a new reliable upload-side signal exists. See lessons.md 2026-05-29 + plan doc `docs/superpowers/plans/2026-05-14-shorts-auto-discovery.md`.
- **Never leave a UI data-load `.catch` as `console.error`-only — set an error state and render a visible line; only show the genuine empty state on a real zero-row success** — because a silently-swallowed load failure renders as a normal-looking empty state ("No clips yet", empty table/schedule) and hid the Clip Library empty-render for ~2 months. Fixed across SettingsView/DataEditorTab/PostingScheduleView (5a985c1, 89063b8, 7532043, 3694460). Same discipline for write/action handlers: surface the failure, don't just log it.
- **Never select `clip_details.thumbnail_base64` from the anon/browser or PostgREST client** — selecting it returns `[]` from the browser supabase-js runtime and `400` from the REST path (same class as the `.not(...is null)` quirk). One bad column fails the whole select; it silently emptied the entire Clip Library list (root cause of the empty-render, fixed 7c32cbb) and 400'd reel-thumbnail downloads. Fetch thumbnails via `thumbnail_url`; the column stays in the DB for server-side use only.
