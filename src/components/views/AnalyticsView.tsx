...
export default function AnalyticsView({ posts }: Props) {
  ...
  const stats = [
    ...
    { label: 'Comments',     value: formatNum(totalComments),      accent: '#F7E7CE' },
    { label: 'Shares',       value: formatNum(totalShares),        accent: '#F7E7CE' },
    ...
    <div className="p-5 space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        {stats.map(({ label, value, accent }) => (
          <div key={label} className="bg-[var(--bg-card)] border border-[rgba(247,231,206,0.06)] rounded-2xl px-4 py-4 hover:border-[rgba(247,231,206,0.09)] transition-colors">
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-[rgba(247,231,206,0.04)]">
            {ALL_PLATFORMS.map((pl) => {
              ...
              return (
                <tr key={pl} className="hover:bg-[rgba(247,231,206,0.02)] transition-colors">
